...
ggplot(data=b_pred_df, mapping=aes(x=solution, fill=true_colour)) + geom_histogram()

ggplot(data=b_pred_df, mapping=aes(x=solution, fill=true_colour)) + geom_density()
```
...
ggplot(data = b_pred_df, mapping = aes(x = EBV, fill = true_colour)) +
  geom_histogram(binwidth = 0.05, color = "black", alpha = 0.7) +
  labs(title = "Distribution of EBVs by True Colour", x = "Estimated Breeding Value (EBV)", y = "Count") +
  theme_minimal()

ggplot(data = b_pred_df, mapping = aes(x = EBV, fill = true_colour)) +
  geom_density(alpha = 0.6) +
  labs(title = "Density of EBVs by True Colour", x = "Estimated Breeding Value (EBV)", y = "Density") +
  theme_minimal()
...
## 7 aug - trying to find the average 'goldness' value per donor


#DONOR GOLD traditional binary include the dplyr average 
...
# Calculate average gold proportion per donor
goldness_summary <- donor_dat_clean %>%
  group_by(donor_id) %>%
  summarise(
    n_pearls = n(),
    mean_gold = mean(Gold, na.rm = TRUE)  # proportion of gold pearls
  ) %>%
  arrange(desc(mean_gold))

# View summary
print(goldness_summary)
...
# Merge BLUPs with goldness summary
merged_df <- BLUP %>%
  left_join(goldness_summary, by = "donor_id")
...
ggplot(merged_df, aes(x = prob_gold, y = mean_gold)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(
    x = "Predicted Probability of Gold (EBV)",
    y = "Observed Proportion of Gold Pearls",
    title = "Donor EBV vs Actual Gold Proportion"
  ) +
  theme_minimal()
...
cor.test(merged_df$prob_gold, merged_df$mean_gold, method = "pearson")
...
uses the raw BLUP (EBV) values instead of the transformed prob_gold values for plotting and correlation analysis
```{r}
# DONOR GOLD
...
# Calculate observed proportion of gold per donor
goldness_summary <- donor_dat_clean %>%
  group_by(donor_id) %>%
  summarise(
    n_pearls = n(),
...
BLUP <- as.data.frame(summary(mod.ind, coef = TRUE)$coef.random)
BLUP$donor_id <- sub(".*donor_id, ainv\\)_", "", rownames(BLUP))
BLUP <- BLUP %>% filter(donor_id %in% unique(donor_dat_clean$donor_id))
BLUP$EBV <- BLUP$solution

head(BLUP)

# 12. Merge BLUPs with observed gold proportion
merged_df <- BLUP %>%
  left_join(goldness_summary, by = "donor_id")

# 13. Plot using EBV instead of prob_gold
library(ggplot2)
...
# 14. Pearson correlation between EBV and observed gold
cor.test(merged_df$EBV, merged_df$mean_gold, method = "pearson")
