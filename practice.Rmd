...
#### compare reseed vs size of pearl/grade/colour
...
# Remove NAs for the relevant columns
data_clean <- data %>% 
  drop_na(Pearl_Harvest_Data_Reseed, 
          Pearl_Grading_Data_Size_mm, 
          Pearl_Grading_Data_Grade, 
          Pearl_Grading_Data_Colour)
...
# Box plot for Pearl Size y Y
ggplot(data_clean, aes(x = Pearl_Harvest_Data_Reseed, y = Pearl_Grading_Data_Size_mm, fill = Pearl_Harvest_Data_Reseed)) +
  geom_boxplot(outlier.color = "red", outlier.size = 2) +
  labs(title = "Pearl Size by Reseed Status", 
       x = "Reseed Status", 
       y = "Pearl Size (mm)") +
  theme_minimal() +
  theme(legend.position = "none")

# ANOVA for Pearl Size
anova_size <- aov(Pearl_Grading_Data_Size_mm ~ Pearl_Harvest_Data_Reseed, data = data_clean)
summary(anova_size)

# Post-hoc test if significant
if (summary(anova_size)[[1]]$`Pr(>F)`[1] < 0.05) {
  library(multcomp)
  posthoc_size <- TukeyHSD(anova_size)
  print(posthoc_size)
  plot(posthoc_size, las = 1, col = "blue")
}



### made the y and Y same for reseed vs pearl size
library(ggplot2)
library(dplyr)

# Standardize the Reseed Status to be consistent
data_clean <- data_clean %>%
  mutate(Pearl_Harvest_Data_Reseed = toupper(Pearl_Harvest_Data_Reseed))
...
if (summary(anova_size)[[1]]$`Pr(>F)`[1] < 0.05) {
  ...
  # Plot the post-hoc results for easier interpretation
  plot(posthoc_size, las = 1, col = "blue")
}
...
# Chi-square test for Pearl Grade
grade_table <- table(data_clean$Pearl_Harvest_Data_Reseed, data_clean$Pearl_Grading_Data_Grade)
grade_test <- chisq.test(grade_table)
grade_test

# Stacked bar plot for Pearl Grade
ggplot(data_clean, aes(x = Pearl_Harvest_Data_Reseed, fill = Pearl_Grading_Data_Grade)) +
  geom_bar(position = "fill") +
  labs(title = "Pearl Grade by Reseed Status", 
       x = "Reseed Status", 
       y = "Proportion", 
       fill = "Pearl Grade") +
  theme_minimal()
...
# Set the correct factor levels for the grade
data_clean <- data_clean %>%
  mutate(Pearl_Grading_Data_Grade = factor(Pearl_Grading_Data_Grade, 
                                           levels = c("A1+", "A1", "A2+", "A2", "A3+", "A3", "A", 
                                                      "B1+", "B1", "B2+", "B2", "LG", "NG"),
                                           ordered = TRUE))

# Plot the data with correct grade ordering
ggplot(data_clean, aes(x = Pearl_Harvest_Data_Reseed, fill = Pearl_Grading_Data_Grade)) +
  geom_bar(position = "fill") +
  labs(title = "Pearl Grade by Reseed Status", 
       x = "Reseed Status", 
...
# Set the correct factor levels for the grade and remove NAs
data_clean <- data_clean %>%
  drop_na(Pearl_Harvest_Data_Reseed, Pearl_Grading_Data_Grade) %>%
  mutate(Pearl_Grading_Data_Grade = factor(Pearl_Grading_Data_Grade, 
                                           levels = c("A1+", "A1", "A2+", "A2", "A3+", "A3", "A", 
                                                      "B1+", "B1", "B2+", "B2", "LG", "NG"),
                                           ordered = TRUE))
...
# Chi-square test for Pearl Colour ##doesn't work
colour_table <- table(data_clean$Pearl_Harvest_Data_Reseed, data_clean$Pearl_Grading_Data_Colour)
colour_test <- chisq.test(colour_table)
colour_test

#remove 0s
# Remove empty rows and columns
colour_table_clean <- colour_table[rowSums(colour_table) > 0, colSums(colour_table) > 0]

# Re-run the test
colour_test_clean <- chisq.test(colour_table_clean)
colour_test_clean

# Chi-Square with Monte Carlo Simulation (if sparse data)
colour_test_mc <- chisq.test(colour_table_clean, simulate.p.value = TRUE, B = 10000)
colour_test_mc

# Double check for NAs
sum(is.na(data_clean$Pearl_Harvest_Data_Reseed))
sum(is.na(data_clean$Pearl_Grading_Data_Colour))
...
# Stacked bar plot for Pearl Colour
library(ggplot2)
library(dplyr)

# Remove NAs and ensure the color column is a factor
data_clean <- data_clean %>%
  drop_na(Pearl_Harvest_Data_Reseed, Pearl_Grading_Data_Colour) %>%
  mutate(Pearl_Grading_Data_Colour = factor(Pearl_Grading_Data_Colour, levels = names(pearl_colors)))

# Create the plot
ggplot(data_clean, aes(x = Pearl_Harvest_Data_Reseed, fill = Pearl_Grading_Data_Colour)) +
  geom_bar(position = "fill") +
  labs(title = "Pearl Colour by Reseed Status", 
       x = "Reseed Status", 
       y = "Proportion", 
       fill = "Pearl Colour") +
  scale_fill_manual(values = pearl_colors) +
...
# Remove NAs for a clean comparison
data_clean <- data_clean %>%
  drop_na(Pearl_Grading_Data_Shape, Pearl_Grading_Data_Colour) %>%
  mutate(Pearl_Grading_Data_Colour = factor(Pearl_Grading_Data_Colour, levels = names(pearl_colors)))

# Create the plot
ggplot(data_clean, aes(x = Pearl_Grading_Data_Shape, fill = Pearl_Grading_Data_Colour)) +
  geom_bar(position = "fill") +
  labs(title = "Pearl Shape by Colour", 
       x = "Pearl Shape", 
       y = "Proportion", 
       fill = "Pearl Colour") +
  scale_fill_manual(values = pearl_colors) +
...
# Define your custom pearl color palette
pearl_colors <- c(
  "Blue" = "#0000FF", 
  "Champagne" = "#F7E7CE", 
  "Cream" = "#FFFDD0", 
...
cleaned_data <- data %>% 
  filter(!is.na(Pearl_Grading_Data_NacreDepositionPearl_mm), 
         !is.na(Pearl_Grading_Data_Colour))

# Boxplot for visual comparison with custom colors
ggplot(cleaned_data, aes(x = Pearl_Grading_Data_Colour, 
                         y = Pearl_Grading_Data_NacreDepositionPearl_mm, 
                         fill = Pearl_Grading_Data_Colour)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.5, color = "black") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Nacre Deposition vs. Colour",
       x = "Pearl Colour",
       y = "Nacre Deposition (mm)") +
  scale_fill_manual(values = pearl_colors)

# ANOVA to test for significant differences
anova_result <- aov(Pearl_Grading_Data_NacreDepositionPearl_mm ~ Pearl_Grading_Data_Colour, 
                    data = cleaned_data)
summary(anova_result)

# Post-hoc test for pairwise comparisons if ANOVA is significant
...
# Define the ordered levels for the grading column
grade_levels <- c("A1+", "A1", "A2+", "A2", "A3+", "A3", "A", 
                  "B1+", "B1", "B2+", "B2", "LG", "NG")

# Convert the grade column to a factor with ordered levels
data$Pearl_Grading_Data_Grade <- factor(data$Pearl_Grading_Data_Grade, levels = grade_levels, ordered = TRUE)

# Remove rows with NA in either column
cleaned_data <- data %>% 
  filter(!is.na(Pearl_Grading_Data_NacreDepositionPearl_mm), 
         !is.na(Pearl_Grading_Data_Grade))

# Boxplot for visual comparison
ggplot(cleaned_data, aes(x = Pearl_Grading_Data_Grade, 
                         y = Pearl_Grading_Data_NacreDepositionPearl_mm, 
                         fill = Pearl_Grading_Data_Grade)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.5, color = "black") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Nacre Deposition vs. Pearl Grade",
       x = "Pearl Grade",
       y = "Nacre Deposition (mm)") +
  scale_fill_manual(values = c(rep("#FFDDC1", 7), rep("#C4A484", 3), "#808080", "#404040")) +
  theme_minimal()

# ANOVA to test for significant differences
anova_result <- aov(Pearl_Grading_Data_NacreDepositionPearl_mm ~ Pearl_Grading_Data_Grade, 
                    data = cleaned_data)
summary(anova_result)
...
cleaned_data <- data %>% 
  filter(!is.na(Pearl_Grading_Data_NacreDepositionPearl_mm), 
         !is.na(Pearl_Grading_Data_Size_mm))

# Scatter plot with linear fit line
ggplot(cleaned_data, aes(x = Pearl_Grading_Data_Size_mm, 
                         y = Pearl_Grading_Data_NacreDepositionPearl_mm)) +
  geom_point(alpha = 0.6, color = "#0072B2") +
  geom_smooth(method = "lm", color = "#D55E00", se = TRUE) +
  labs(title = "Nacre Deposition vs. Pearl Size",
       x = "Pearl Size (mm)",
       y = "Nacre Deposition (mm)") +
  theme_minimal()

# Linear regression model to quantify the relationship
lm_model <- lm(Pearl_Grading_Data_NacreDepositionPearl_mm ~ Pearl_Grading_Data_Size_mm, 
               data = cleaned_data)
summary(lm_model)

# Correlation coefficient
correlation <- cor(cleaned_data$Pearl_Grading_Data_NacreDepositionPearl_mm, 
                   cleaned_data$Pearl_Grading_Data_Size_mm)
cat("\nPearson Correlation Coefficient:", correlation, "\n")




####nacredepositionkeshi vs size/colour/grade
...
# Define ordered levels for the Grade column
grade_levels <- c("A1+", "A1", "A2+", "A2", "A3+", "A3", "A", 
                  "B1+", "B1", "B2+", "B2", "LG", "NG")
data$Pearl_Grading_Data_Grade <- factor(data$Pearl_Grading_Data_Grade, 
                                        levels = grade_levels, ordered = TRUE)

# Remove rows with NA in the key columns
cleaned_data <- data %>% 
  filter(!is.na(Pearl_Grading_Data_NacreDepositionKeshi_mm), 
         !is.na(Pearl_Grading_Data_Colour), 
         !is.na(Pearl_Grading_Data_Size_mm), 
         !is.na(Pearl_Grading_Data_Grade))
...
ggplot(cleaned_data, aes(x = Pearl_Grading_Data_Colour, 
                         y = Pearl_Grading_Data_NacreDepositionKeshi_mm, 
                         fill = Pearl_Grading_Data_Colour)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.5, color = "black") +
  scale_fill_manual(values = pearl_colors) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Keshi Nacre Deposition vs. Colour",
       x = "Pearl Colour",
       y = "Keshi Nacre Deposition (mm)") 





# 2. Scatter Plot for Keshi Nacre vs Size
ggplot(cleaned_data, aes(x = Pearl_Grading_Data_Size_mm, 
                         y = Pearl_Grading_Data_NacreDepositionKeshi_mm, 
                         color = Pearl_Grading_Data_Grade)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(title = "Keshi Nacre Deposition vs. Pearl Size",
       x = "Pearl Size (mm)",
       y = "Keshi Nacre Deposition (mm)") +
  theme_minimal()

# 3. Boxplot for Nacre Thickness (Keshi) vs Grade
ggplot(cleaned_data, aes(x = Pearl_Grading_Data_Grade, 
                         y = Pearl_Grading_Data_NacreDepositionKeshi_mm, 
                         fill = Pearl_Grading_Data_Grade)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.5, color = "black") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Keshi Nacre Deposition vs. Grade",
       x = "Pearl Grade",
       y = "Keshi Nacre Deposition (mm)") +
  theme_minimal()
...
cor_data <- cleaned_data[, c("Pearl_Grading_Data_NacreDepositionKeshi_mm", "Pearl_Grading_Data_Size_mm")]
...
# 5. ANOVA for Grade
anova_result <- aov(Pearl_Grading_Data_NacreDepositionKeshi_mm ~ Pearl_Grading_Data_Grade, 
                    data = cleaned_data)
summary(anova_result)
...
# Remove rows with NA in the key columns
cleaned_data <- data %>% 
  filter(!is.na(Pearl_Grading_Data_Price_AUD), 
         !is.na(Pearl_Grading_Data_Colour), 
         !is.na(Pearl_Grading_Data_Size_mm), 
         !is.na(Pearl_Grading_Data_Grade))

# 1. Boxplot for Price vs Colour
ggplot(cleaned_data, aes(x = Pearl_Grading_Data_Colour, 
                         y = Pearl_Grading_Data_Price_AUD, 
                         fill = Pearl_Grading_Data_Colour)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.5, color = "black") +
  scale_fill_manual(values = pearl_colors) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Pearl Price (AUD) vs. Colour",
       x = "Pearl Colour",
       y = "Price (AUD)") 

# 2. Scatter Plot for Price vs Size
ggplot(cleaned_data, aes(x = Pearl_Grading_Data_Size_mm, 
                         y = Pearl_Grading_Data_Price_AUD, 
                         color = Pearl_Grading_Data_Grade)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", color = "black", se = TRUE) +
  labs(title = "Pearl Price (AUD) vs. Pearl Size",
       x = "Pearl Size (mm)",
       y = "Price (AUD)") +
  theme_minimal()

# 3. Boxplot for Price vs Grade
ggplot(cleaned_data, aes(x = Pearl_Grading_Data_Grade, 
                         y = Pearl_Grading_Data_Price_AUD, 
                         fill = Pearl_Grading_Data_Grade)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.5, color = "black") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Pearl Price (AUD) vs. Grade",
       x = "Pearl Grade",
       y = "Price (AUD)") +
  theme_minimal()

# 4. Correlation Matrix (Size and Price)
cor_data <- cleaned_data[, c("Pearl_Grading_Data_Price_AUD", "Pearl_Grading_Data_Size_mm")]
cor_data <- na.omit(cor_data)
cor_matrix <- cor(cor_data)
print(cor_matrix)

# 5. ANOVA for Grade
anova_result <- aov(Pearl_Grading_Data_Price_AUD ~ Pearl_Grading_Data_Grade, 
                    data = cleaned_data)
summary(anova_result)
...
#### saibo area vs matched donor data saibo area round/flat number regions
...
# Remove rows with NA in the key columns
cleaned_data <- data %>% 
  filter(!is.na(First_Op_Data_Saibo_Area), 
         !is.na(Matched_Donor_Data_Saibo_Area_Round_NumberRegions), 
         !is.na(Matched_Donor_Data_Saibo_Area_Flat_NumberRegions))

# 1. Scatter Plot for Saibo Area vs Round Regions
ggplot(cleaned_data, aes(x = Matched_Donor_Data_Saibo_Area_Round_NumberRegions, 
                         y = First_Op_Data_Saibo_Area)) +
  geom_point(alpha = 0.7, color = "#0072B2") +
  geom_smooth(method = "lm", color = "#D55E00", se = TRUE) +
  labs(title = "Saibo Area vs. Round Number of Regions",
       x = "Round Number of Regions",
       y = "Saibo Area (mm²)")

# 2. Scatter Plot for Saibo Area vs Flat Regions
ggplot(cleaned_data, aes(x = Matched_Donor_Data_Saibo_Area_Flat_NumberRegions, 
                         y = First_Op_Data_Saibo_Area)) +
  geom_point(alpha = 0.7, color = "#009E73") +
  geom_smooth(method = "lm", color = "#E69F00", se = TRUE) +
  labs(title = "Saibo Area vs. Flat Number of Regions",
       x = "Flat Number of Regions",
       y = "Saibo Area (mm²)") +
  theme_minimal()
...
cleaned_data <- cleaned_data %>%
  mutate(across(c(First_Op_Data_Saibo_Area, 
                  Matched_Donor_Data_Saibo_Area_Round_NumberRegions, 
                  Matched_Donor_Data_Saibo_Area_Flat_NumberRegions), 
                as.numeric))

# Prepare data for correlation
cor_data <- cleaned_data[, c("First_Op_Data_Saibo_Area", 
                             "Matched_Donor_Data_Saibo_Area_Round_NumberRegions", 
                             "Matched_Donor_Data_Saibo_Area_Flat_NumberRegions")]
...
# Create a heatmap for the correlation matrix ##might be useless
...
ggplot(melted_cor_matrix, aes(Var1, Var2, fill = value)) +
  ...
  labs(title = "Correlation Matrix for Saibo Area and Donor Regions",
       x = "",
       y = "") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Cross-tabulation: First Op Saibo Area vs Round Number Regions
table_round <- table(data$First_Op_Data_Saibo_Area, data$Matched_Donor_Data_Saibo_Area_Round_NumberRegions)
print(table_round)

# Chi-squared test
chisq_round <- chisq.test(table_round)
print(chisq_round)

# Cross-tabulation: First Op Saibo Area vs Flat Number Regions
table_flat <- table(data$First_Op_Data_Saibo_Area, data$Matched_Donor_Data_Saibo_Area_Flat_NumberRegions)
print(table_flat)

# Chi-squared test
chisq_flat <- chisq.test(table_flat)
print(chisq_flat)

# Visualize with mosaic plots

# Mosaic plot for Round Number Regions vs Saibo Area
mosaicplot(table_round, color = TRUE,
           main = "Mosaic Plot: Saibo Area vs Round Number of Regions")

# Mosaic plot for Flat Number Regions vs Saibo Area
mosaicplot(table_flat, color = TRUE,
           main = "Mosaic Plot: Saibo Area vs Flat Number of Regions")
